...
const OAUTH_ERROR_MESSAGES: Record<string, string> = {
  ...
  google_no_email:
    "Google didn't share an email address, so we couldn't sign you in. Try a different account.",
};
...
function OAuthErrorBanner({ onError }: { onError: (message: string) => void }) {
  ...
  useEffect(() => {
    ...
    if (code) {
      onError(OAUTH_ERROR_MESSAGES[code] ?? "Something went wrong signing you in.");
    }
