...
export default function UploadPage() {
  ...
  useEffect(() => {
    api
      ...
      .then(async () => {
        ...
        setDocuments(docs);
        if (docs.length === 0) setShowUploader(true);
      })
  ...
  if (checkingAuth || documents === null) {
    return (
  ...
  const hasReports = documents.length > 0;
  ...
  return (
    <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col px-6 py-14">
      ...
      <p className="mt-2 text-sm text-ink-soft">
        {hasReports
          ? "Your inspection reports, all in one place."
      ...
      {error && (
        <div
          role="alert"
          className="mt-6 rounded-2xl border border-brick/20 bg-brick-soft px-5 py-4 text-sm text-brick"
        >
          {error}
          <button
            type="button"
      ...
      {!showUploader && hasReports && (
        <div className="mt-10 mb-5 flex items-center justify-between">
          ...
          <button
            ...
            className="rounded-full bg-accent-soft px-3.5 py-1.5 text-xs font-medium text-accent transition-colors hover:bg-accent hover:text-accent-ink"
          >
            + New report
          </button>
        </div>
          ...
          {documents.map((document) => (
            <li key={document.id}>
              <Link
                ...
                className="flex items-center justify-between gap-4 rounded-2xl border border-line bg-surface px-6 py-5 text-sm transition-colors hover:border-accent"
              >
                <div className="min-w-0">
                  <p className="truncate font-medium text-ink">{document.title}</p>
                  <p className="mt-1 text-xs text-ink-faint">{formatDate(document.created_at)}</p>
                </div>
        ...
        <div className="mt-10">
          {hasReports && !busy && (
            <button
              ...
              className="mb-5 text-xs font-medium text-ink-faint underline underline-offset-2 hover:text-ink"
            >
